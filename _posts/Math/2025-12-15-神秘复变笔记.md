...
那我们可以换元，$d \theta = \frac 1 {iz} dz$，构造圆形围道。
...
接下来有一个洞察：任意 Laurent 级数在求导后一定不含 $-1$ 次项，因此任意导数形式的东西留数都是 $0$。

我们构造

$$\frac {x f'(x)} {f(x)^{n+1}} = \frac 1 n \frac 1 {f(x)^n} \textcolor{red}{- \frac 1 n \frac d {dx} (\frac x {f(x)^n})}$$
