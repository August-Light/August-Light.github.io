...
$$I'(a) = \int_{-\infty}^\infty (-x^2) e^{-ax^2} dx = - \frac {\sqrt \pi} {2 a^{\frac 3 2}}$$
...
一个满足正态分布的随机向量 $\vec{x}$ 满足期望为 $\vec{\mu}$ 且协方差矩阵为 $\Sigma$，则其 PDF 为：
...
> 构造一个卷积核，通过卷积的方式做到模糊图像的效果。

很显然为了模糊图像，我们需要把每个像素和周围的像素取平均，越靠近该像素权值越高。

为了起到均匀的模糊效果，我们需要一个各向同性的权值分布。

为了方便卷积计算，我们最好要一个各维互相独立的权值分布。具体来说，对于各维独立的卷积核，2D 的卷积可以被拆成 $2$ 次 1D 的卷积。

此时答案很显然了——我们需要一个正态分布，这就是高斯模糊的来源。

注：当卷积核很大的时候，“各维独立”这一性质就没什么用了，因为可以直接 FFT。但是高斯核往往很小，所以这样的优化是有效的。
