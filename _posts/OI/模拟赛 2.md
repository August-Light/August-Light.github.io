...
## [P1393 Mivik 的标题](https://www.luogu.com.cn/problem/P1393)🟡✅️ (Hard)

> 给定一个长为 $m$ 的模式串 $T$ 和字符集大小 $\lvert \Sigma \rvert$。求有几个长为 $n$ 的主串 $S$ 包含该模式串。
>
> 答案对 $998244353$ 取模。
>
> $n \le 2 \times 10^5$，$m \le 10^5$，$\lvert \Sigma \rvert \le 10^8$。
>
> 注：本题有一个简单版本 [P3193 [HNOI2008] GT考试](https://www.luogu.com.cn/problem/P3193)，$n \le 10^9$，$m \le 20$，$\lvert \Sigma \rvert = 10$。

很容易想到建 KMP 自动机，然后矩阵快速幂优化做到 $O(m \lvert \Sigma \rvert + m^3 \log n)$，这是简单版的做法。

但是这个做法对我们现在这个题完全没用，因为 $m$ 非常大，根本不可能建出 KMP 自动机。

我们考虑普通的 DP。设 $f_i$ 为 $T$ 在 $S_{[1,i]}$ 的后缀处**第一次出现**的方案数。最终答案为 $\sum_{i=m}^n f_i \lvert \Sigma \rvert^{n-i}$。

怎么 DP？我们考虑在确定 $T$ 是后缀的情况下乱填的方案数是 $\lvert \Sigma \rvert^{i-m}$，但是要扣除不合法情况。

Case 1：$T$ 的首次出现和后缀 $T$ 完全不相交。枚举其位置 $j$ 可知，需要扣除的方案数为

$$\sum_{j=m}^{i-m} f_j \lvert \Sigma \rvert^{i-m-j}$$

提出来 $\lvert \Sigma \rvert^i$ 后，显然可以前缀和优化。

Case 2：$T$ 的首次出现和后缀 $T$ 相交，那么**相交部分是一个 border**。要扣除的方案数为

$$\sum_{j \text{ is a border of } T} f_{i-m+j}$$

这一步是瓶颈，因为 border 个数是 $O(m)$ 的。

时间复杂度 $O(nm)$。过不了。

回忆一个性质：$T$ 的所有 border 构成 $O(\log m)$ 个等差数列。我们对于每个等差数列维护即可。

具体来说，令等差数列首项为 $l$ 末项为 $r$ 公差为 $d$。维护前缀和数组 $sum_i = \sum_{k} f_{i - kd}$，则这个等差数列对 Case 2 答案的贡献为

$$sum_{i-m+r} - sum_{i-m+l-d}$$
