...
注意到，当 DP 式子是 $\min$ 时，这个点云上只有下凸壳会造成贡献。（同理 $\max$ 时只有上凸壳会造成贡献）每次查询时可以二分得到最优决策点。

TODO: 维护凸包

不难发现斜率优化解决的问题类型和决策单调性优化非常相似，但是斜率优化通过更特殊的 DP 式子有时可以获得更好的复杂度，或是解决不满足决策单调性的问题（？TODO:）。
...
注意到 $k(i) = 2 (s_i - L)$ 单调递增，可以使用单调队列维护。时间复杂度 $O(n)$。