...
- $*$，Dirichlet 卷积：$(f * g)(x) = \sum\limits_{d \mid n}f(d)g(\dfrac n d)$。
...
- 每个数论函数 $f$（除了 $f(1) = 0$ 的）都有逆元（即 $f * g = \varepsilon$）：
  - $g(n) = \dfrac 1 {f(1)} \left([n = 1] - \sum\limits_{i \mid n, i \ne 1} f(i)g\left(\dfrac n i \right)\right)$
  - $f$ 的逆元记作 $f^{-1}$。
...
[[Luogu P5495]【模板】Dirichlet 前缀和](https://www.luogu.com.cn/problem/P5495)：已知 $f$，对于 $1$ 到 $n$ 的所有 $i$，求出 $g_i = \sum\limits_{d \mid i} f_d$。要求 $O(n \log \log n)$。
...
- Dirichlet 前缀和（高维前缀和）
  - $g_i = \sum\limits_{d \mid i} f_d$
- Dirichlet 前缀差分（前一个的逆运算）
- Dirichlet 后缀和（高维后缀和）
  - $g_i = \sum\limits_{i \mid d} f_d$
...
- $\sigma_k(ij) = \sum\limits_{x \mid i} \sum\limits_{y \mid j} [x \perp y] (\dfrac {xj} y)^k$
...
$$f_p(x) = \sum\limits_{i=0}^\infty f(p^i) x_i$$
