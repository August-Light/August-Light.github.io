...
btw 这题点分治也可以做（也是 1log），但是静态链分治明显更好写不是吗！
...
TODO: 合并 $n$ 个单元素动态开点线段树的时间复杂度是 $O(n \log n)$ 的。

TODO:

Ref: [算法学习笔记(88): 线段树合并 - Pecco](https://zhuanlan.zhihu.com/p/575513452)
...
显然时空会爆，但是这个思路是可行的。我们只要对叶子用平衡树维护桶，然后每次递归回来的时候**启发式合并**（因为 $l,r$ 的桶对后面的计算无用，直接合起来当 $u$ 的桶），时间复杂度 $O(n \log^2 n)$。

2log 不够猛，看一下 1log 做法。我们改用线段树维护桶，然后线段树合并。
...
- 往集合 $S$ 插入一个二进制数
- 给定一个 $x$，求 $\sum\limits_{y \in S} [x \sube y]$。$x \sube y$ 代表 $x \land y = x$。
