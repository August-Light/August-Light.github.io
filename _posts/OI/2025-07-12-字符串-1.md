...
<img src="https://cdn.luogu.com.cn/upload/image_hosting/lb4bk6u8.png" width=400/>
...
证明见[双模数结构、周期引理和同余最短路](https://www.luogu.com.cn/article/cz8shjvu)。
...
> 给定一个字符串 $S$。
>
> 对于长度 $\ge \lfloor \frac {\lvert S \rvert} 2 \rfloor$ 的 border，它们和 $\lvert S \rvert$ 共同构成一个**等差数列**。
>
...
> 定义：偶串是形如 $A+A$ 的串。
> 对于一个偶串 $S+S$，$f(S+S)$ 是满足以下条件的最短字符串：
> - $f(S+S)$ 是偶串
> - $S+S$ 是 $f(S+S)$ 的前缀
