...
## [P4331 [BalticOI 2004] Sequence (Day1)](https://www.luogu.com.cn/problem/P4331) (Hard)
...
上凸函数取前缀 $\max$ 还是上凸函数。可以看成与 $f(x) = 0$ 的 $(\min, +)$ 卷积。根据几何意义 Minkowski 和，其实就是丢弃了斜率为负的部分，全变成平的了。

一个比较取巧的实现就是二分 $\lambda$ 时直接把二分下界取成 $0$。
